...
class StatRendererView{
	...
	updateResult(data){
		...
		var gn=0;this.hovergroups=[];
		for(var t in this.statresult) {
			var grs=this.statresult[t].groups;
			for(let g in grs) if(grs[g].included) {gn++;this.hovergroups.push(grs[g]);}	//update the groups that are interactive
		}
	...
	buildTableDataObj(){
		...
		var alltabledata=[];
		for(var t in this.statresult){
			let dat=this.statresult[t], gh=this.getGroupHeaders(dat.groups);
			var tabledata={};
			...
			tabledata.headers.push("All","Sign.Diff","Ordered");
			...
			for (var v in vars) {
				...
				for(var g in dat.groups) {
					...
					if(fobj) line.push(fobj.average.toPrecision(2)+String.fromCharCode(97 +fobj.signifgroup)+" (n:"+fobj.n+")");
				}
...
class StatListView{
	constructor(rootcomp,classifier) {
		...
		this.blockinfonames=["id","species","date","substrate","totalharvest","totalBE"];
	...
	showList(data,title){
		...
		let tabledata= this.prepareData(data,title);
		this.tablecomp.setTableContent(tabledata);
	...
	prepareData(data,title){
		var res={};
		res.caption=title;
		res.headers=this.blockinfonames.concat(this.varnames);//[""].concat(this.blockinfonames);
		res.headers=removeDuplicates(res.headers);
		res.rows=[];
		for(let i in data) {
			let dv=data[i],line=[];
			for(let n in res.headers) {
				...
				if(!val){
					val=dv.getInfo(res.headers[n]);
				}
				val=this.formatValue(val);
				line.push(val);
			}
			line.push(this.classifier.getBlockGroups(dv.getInfo("id")));
			res.rows.push(line);
...
class StatBlockView {
	...
	//	this.viewcomps.br4=new SimpleBR(this.viewcomps.infos,"statblockbr4");
		...
		this.viewcomps.calcsfirstflushtitle.setStyle("font-size","50%");
		...
		this.viewcomps.calcsfirstflushBEtitle.setStyle("font-size","50%");
		...
		this.viewcomps.history.setStyle("font-size","75%");
	...
	showBlock(block){
		...
		this.viewcomps.infosdate.setText(minusifnotfound("date"));
		...
		let tsub2=minusifnotfound("totalBE");
