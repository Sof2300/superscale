...
class InteractiveMultiTable extends MultiTable {
	...
	createSubTable(){
		let table= new InteractiveTable(this);
...
// getters / setters
	...
	createCell(parent,value,row,column, tag){
		var cell = new InteractiveCell(parent, this.makeCellId(row,column), tag, this);
	//	if(row!=)
	...
	makeInteractive(row,column){	//table, row, column
		let i=this.cellindex[row][column].interacter;
		if(i) {
			i.clearModels();
			i.addModel(this.selectmodel);
...
class InteractiveItem extends ComponentItem {
	constructor(parent, id) {
		...
		if(mod) {this.userinput=this.element.value;this.text=this.uservalue;}//hello
		if(mod || this.toremove) {
			this.setStyle("backgroundColor",this.activebgcolor);
...
setToRemove(b){this.toremove=b;this.updateBG();}	// means this item does not exist anymore in remote model, happens when user changed it
	...
	updateText(v, storeonly){	// this does not show the update if user modified the input or if told
		if(!this.isUserModified() && !storeonly) {this.setText(v);this.remotevalue=undefined;}	// if not usermodified
...
class Input extends InteractiveItem{
	constructor(parent, id){
		super(parent, id);
		this.setTag("input");
		this.setAutoSize(1);// looks like a good idea
	...
	setAutoSize(turnon){	//should be on update too, onchange
		if(turnon) {
			this.addListener("onkeydown", this.autoSize.bind(this));
			this.addListener("onchange", this.autoSize.bind(this));
		}
		else {
			this.removeListener("onkeydown", this.autoSize.bind(this));
	...
	autoSize(){
		...
		if(content.length >minsize){
			this.setStyle("width", (content.length +1) + "ch");
		} else this.setStyle("width", (minsize +1) + "ch");
		this.busy=false;
	}
...
class CheckBox extends InteractiveItem{
	...
	updateBG(){	// should we save the value of the user input in this object ?
		...
		if(mod || this.toremove) this.element.style["filter"]=activebgcolorfilter;	
		else this.element.style["filter"]=null;	
	}
	...
	isUserModified(){
		...
		if(this.remotevalue && r!==this.element.checked) return true;
		if(!this.remotevalue && this.element.checked!==v) return true;
		return false;
	}
	/*	callback(){
