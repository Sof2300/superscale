...
class Classifier {
    ...
    getAllBlockList(){
        var blocklist=[];
        for(let t in this.tests) for(let g in this.tests[t].groups) {
            var found=0, glist=this.tests[t].groups[g].list
            for(var b in blocklist) {
                for(var ge in glist)
                    if(glist[ge]==blocklist[b]) {found=1;break;}
                if(found) break;
            }
            if(found) continue;
            blocklist=blocklist.concat(glist);
        }
        return blocklist;
    }
    ...
    parse(testgroups){
        ...
        for(var l in lines) {
            ...
            if (vs < 0) continue;
            b = true;
            let tname = "T" + line.substring(0, vs);
            ...
            for(var s in sublines) sublines[s]=sublines[s].trim();
            var cobj=this.tests[tname];
            if (gs < 0 || gs > vs) cobj=this.tests[tname];
            else {
                ...
                cobj=this.tests[tname].groups[gname];
            }
            cobj.setDescription(sublines[0]);
            for (let cl = 1; cl < sublines.length; cl++){
                cobj.addCondition(sublines[cl]);    //this.tests[tname].addParameter(sublines[cl]);
                cobj.addParameter(sublines[cl]);
            }


      /*      if (gs < 0 || gs > vs) {    // when is it supposed to happen ? anova test level - what is it supposed to do ?
                let cline = line.substring(vs + 1);
                let sublines = cline.split(nl);
                for(var s in sublines) sublines[s]=sublines[s].trim();
                this.tests[tname].setDescription(sublines[0]);
                for (let cl = 1; cl < sublines.length; cl++) this.tests[tname].addParameter(sublines[cl]);      // we do not add condition ?
            } else {
                ...
                for (let cl = 1; cl < sublines.length; cl++){
                    this.tests[tname].groups[gname].addCondition(sublines[cl]);    //this.tests[tname].addParameter(sublines[cl]);
                    this.tests[tname].groups[gname].addParameter(sublines[cl]);
                }
            }*/
        }
        return b;
