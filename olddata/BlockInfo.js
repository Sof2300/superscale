...
class BlockInfo extends StatDataSample {
    ...
    //    this.info.substrate = this.substratetext;
        ...
        for(let i in this.flushinfos){ let j=i;
            ...
            if (info.date) info[flushnames[j]+"flushdays"] = this.flushinfos[0].ts - info.date;
        }
    ...
    calcFlushes() {
        ...
        function fuseFlushes(flushes) {
           ...
           while (i +1< flushes.length) {
                var vi = flushes[i];
                var vj = flushes[i + 1]
                if (Math.abs(vj.ts - vi.ts) < MAXFLUSHDAYS*SECONDSPERDAY) {
        ...
        for (let line in fh) {
            ...
            if (BlockInfo.isHarvest(fl)) {
                let val = BlockInfo.retreiveArgument(fl);this.flushinfos.push({ts: BlockInfo.timestampFromString(BlockInfo.retreiveDate(fl)), val: val});
                this.totalharvest += val;
            }
        }
        if(this.flushinfos.length==0) return;this.flushinfos = orderFlushes(this.flushinfos);// order the list by date
