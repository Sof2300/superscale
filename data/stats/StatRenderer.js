...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// to move in group definition parsing file
...
class StatFilterView{
	...
	//	this.basecomponent.setStyle("padding","8px");
		...
		this.basecomponent.setStyle("margin-bottom","8px");
		...
		this.filterbuttoncomp.setStyle("font-size","90%");
...
//		if(e.keyCode==13 && m) this.rendererview.updateFilters(this.filtervalcomp.element.value);
	...
	getFilterText(){
		var text=this.filtervalcomp.element.value
	...
	buttonpress() {
		...
		let m = this.checkSyntax(text);
		if (!m) {
			this.rendererview.updateFilters(text);
...
class StatVarSelectView {
	...
	//	this.basecomponent.setStyle("border","solid");
		this.basecomponent.setStyle("border-width","1px");
	//	this.basecomponent.setStyle("padding","5px");
		...
		this.varselectcomp.setStyle("font-size","70%");
...
function decimalLimit(oval,limit){
	let val=oval, f=1;
	for(let i=limit;i>0;i--) f=f*10;
	return Math.round(oval*f)/f;
}
...
class StatRendererView{
	constructor(rootelem,classifier,allvarnames) {
		...
		this.buildView();
		this.detailview = new StatBlockView(this.componentrootnode);
		this.listview = new StatListView(this.componentrootnode, classifier);
		this.listview.detailview=this.detailview;
	...
	updateResult(data){
		...
		for(var t in tabledata){
			if(!tabledata[t].hovergroups) continue;
			var l=0;
		...
		for(var t in this.tablecomp.tables)
		if(tabledata[t].rows) for(let i=0;i<tabledata[t].rows.length;i++) {
			let c=this.tablecomp.tables[t].getCellComp(i+1,tabledata[t].rows[i].length-1);	//change style of last cell
			if(c) c.setStyle("font-size","80%");
			else console.log();
		}
//		var gn=0;
//		for(var i=1;i<(gn+2);i++) this.tablecomp.makeInteractive(0,0,i);
	...
	buildView() {
		var lbut,ltab, visval=true;
		this.testbutton=new Button(this.componentrootnode,"stattable");
		lbut=this.testbutton;
		lbut.setText("Stat tests result \u25bc");
		lbut.setStyle("margin-top","5px");
		lbut.setStyle("margin-left","0px");
		lbut.setStyle("margin-bottom","0px");
		lbut.setStyle("font-size","80%");
		lbut.setStyle("font-weight","bold");
		this.tablecomp=new InteractiveMultiTable(this.componentrootnode,"stattable");
		ltab=this.tablecomp;
		this.tablecomp.setStyle("font-size","65%");
		this.tablecomp.setStyle("width","100%");

		this.tablecomp.setSelectListener(this.selected.bind(this));
		lbut.setStyle("background","white");
		lbut.setStyle("border-color","white");
		this.testbutton.addListener("onclick",function(){
			visval=!visval;
			if(visval) {ltab.setStyle("display","block");lbut.setStyle("color","black");lbut.setText("Stat tests result \u25bc");}
			else {ltab.setStyle("display","none");lbut.setStyle("color","gray"); lbut.setText("Stat tests result \u25ba");}
				});
	}
	...
	buildTableDataObj(){
		...
		for(var t in this.statresult){
			let dat=this.statresult[t];
			if(!dat.data.length) {
				var obj={};
				//obj.caption=dat.title+" match no data";
				obj.headers=[dat.title+" has no data match"];
				alltabledata.push(obj);continue;}
			var gh=this.getGroupHeadersWithCount(dat.groups);
			...
			let count=0;
			for(let d of dat.data) if(d.count>count) count=d.count;
//			if(dat.data.length>0) count=dat.data[0].count;
			tabledata.headers.push("All (n:"+count+")","Sign.Diff","Ordered");
			...
			for (var v in vars) {
				...
				for(var g in dat.groups) {
					...
					if(fobj) line.push(decimalLimit(fobj.average,2)+String.fromCharCode(97 +fobj.signifgroup));
					else if(dat.groups[g].included) line.push("");
					if(dat.groups[g].included) {tabledata.hovergroups[includedcount]=dat.groups[g];includedcount++;}
				}
				let alltext=decimalLimit(vobj.average,2);//.toPrecision(2);//+" (n:"+vobj.count+")";
				line.push(alltext);
				let fdiff="",comp="<";
				if(vobj.signifdiff) {
					fdiff+= "YES";
				...
				fdiff+=" ("+decimalLimit(vobj.testF,3)+comp+decimalLimit(vobj.tableF,3)+")";
				line.push(fdiff);
				let ordtext="";
...
class StatListView{
	constructor(rootcomp,classifier) {
		...
		this.blockinfonames=["id","species","blockdate","formula","totalharvest","totalBE"];
	...
	updateDisplay(anovaname,groupname){
		...
		if(!anovaname) {
			let alldata=this.classifier.getAllBlockList();
			this.showList(alldata,"All tests - All groups ("+alldata.length+")");return;}	//with all blocks
		...
		if(groupname && blocklist.length>0)  {
			...
			let groupdesc=groupobj.short+": "+groupobj.description;
			let t=new Date().getTime();
			this.showList(blocklist,anovaname+":"+anovadesc+" - "+groupdesc+" ("+blocklist.length+")");
			...
			let t=new Date().getTime();
			this.showList(blocklist,anovaname+":"+anovadesc+" - All"+" ("+blocklist.length+")");
			let t2=new Date().getTime();
			console.log("group change time:"+(t2-t));
		}
	...
	showList(data,title){
		...
		for(let i=0;i<tabledata.rows.length;i++) {
			let c=this.tablecomp.getCellComp(i+1,tabledata.rows[i].length-1);	//change style of last cell
			if(c) c.setStyle("font-size","80%");
		}

		for(var k in tabledata.headers) this.tablecomp.makeInteractive(0,Number(k),this.extramodel);
	}
	testDate(b1){ return b1.indexOf("/")>0 && b1.indexOf("/")!=b1.lastIndexOf("/");}
	getDateText(text){
		var a = text.indexOf("/"), b=text.lastIndexOf("/");
		var days=text.substring(0,a);
		var months=text.substring(a+1,b);
		return months+"/"+days+text.substring(b);
	}
	...
	sort(data,columnname){
		...
		while(bubbling){
			...
			for(var i=0;i<data2.length;i++) if(i+1<data2.length){
					...
					if(typeof b1=="string") {
						if(this.testDate(b1)) {var t=this.getDateText(b1);
							b1=new Date(this.getDateText(b1));
							if(this.testDate(b2)) b2=new Date(this.getDateText(b2));
							b=b1<b2
						} else if(typeof b2=="string") b=b1.toLowerCase()<b2.toLowerCase();else b1=1;}
					if(typeof b1=="number") {
						if(typeof b2=="string") b2=-1;
						b=b1<b2;
					}
				//	console.log("i:"+i+" :"+b);
					if(b) {toswap=i;break;}
	...
	formatValue(val){
		if(val && typeof val=="number") {if((val-Math.floor(val)>0)) val=decimalLimit(val,2);}
...
class StatBlockView {
	...
	//	this.titlecomponent=new SimpleText(rootcomp,"detailviewtitle");
	//	this.titlecomponent.setText("Details:");
		...
		this.basecomponent.setStyle("margin-top","10px");
		this.viewcomps={};
		...
		this.viewcomps.infosformulatitle=new Span(this.viewcomps.infos,"statblockinfosdatetitle");
		this.viewcomps.infosformulatitle.setStyle("font-size","55%");
		this.viewcomps.infosformulatitle.setText("Formula:");
		this.viewcomps.infosformula=new Span(this.viewcomps.infos,"statblockinfosformula");
		this.viewcomps.infosformula.setStyle("font-size","90%");
		this.viewcomps.br21=new SimpleBR(this.viewcomps.infos,"statblockbr21");
	...
	showBlock(block){
		...
		this.viewcomps.infosformula.setText(minusifnotfound("formula"));
		...
		if(typeof tsub =="number") tsub=decimalLimit(tsub,1)+"%";
		...
		let tsub2=minusifnotfound("firstflushBE");
		if(typeof tsub2 =="number") tsub2=decimalLimit(tsub2,1)+"%";
